...
function Navbar() {
  ...
  return (
    ...
    <div className={`fixed w-full z-10 ${visible ? 'block' : 'hidden'} fixed z-[999] w-full px-20 py-8 flex justify-between items-center font-["Neue_Montreal"]`}>
        <div className='logo'>
        ...
        <div className='links flex gap-10 hover:cursor-pointer'>
            {["Services","Our works","About Us","Insights","Contact Us"].map((items,index)=>(
            <a key={index} className={`text-md capitalize font-light ${index==4 && 'ml-40'}`}>{items}</a>
            ))}
